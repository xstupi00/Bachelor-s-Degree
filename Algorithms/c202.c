...
*/
	if (s)
		s->top = -1;		// setting top of the stack
	else
		stackError(SERR_INIT);	// call error status
}
...
*/
	  return ( s->top == -1 ? 42 : 0 ); // control of emptiness
}
...
*/
	return ( s->top == STACK_SIZE - 1 ? 42 : 0);	// control of fullness
}
...
*/
	if ( !(stackEmpty(s)) )
		*c = s->arr[s->top];	// returns top of the stack
	else
		stackError(SERR_TOP); 	// call error status
}
...
*/
	if ( !(stackEmpty(s)) )
		s->top--;	// removes element from the top of the stack
}
...
*/
	if ( !(stackFull(s)) )
		s->arr[++s->top] = c; 	// inserts element on the top of the stack
	else
		stackError(SERR_PUSH); 	// call error status
}
