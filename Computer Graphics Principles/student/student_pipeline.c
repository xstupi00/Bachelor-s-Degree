...
VertexIndex gpu_computeGLVertexID(
  ...
  if (indices != NULL)  // indexing is not active
    return indices[vertexShaderInvocation];
  return vertexShaderInvocation; // indexing is active
}
...
void const *gpu_computeVertexAttributeDataPointer(
  ...
  if (head->enabled) // head is enabled
     	return (size_t*)((size_t)head->buffer + (gl_VertexID*head->stride) + head->offset);
  return NULL;   // returns NULL, if head is not enabled
}
...
void gpu_runVertexPuller(GPUVertexPullerOutput *const              output,
  ...
  // precomputation the new VertexIndex
  VertexIndex newVertexID = gpu_computeGLVertexID(puller->indices, vertexShaderInvocation);
  // run for all atrributes
  for (unsigned i = 0; i < MAX_ATTRIBUTES; i++)
    output->attributes[i] = gpu_computeVertexAttributeDataPointer(puller->heads+i, newVertexID);
}
...
void gpu_runPrimitiveAssembly(
  ...
  GPUVertexPullerOutput pullerOutput; // output of PULLER
  GPUVertexShaderInput shaderInput;   // input of SHADER
  for (unsigned i = 0; i < VERTICES_PER_TRIANGLE; i++) {
    gpu_runVertexPuller(&pullerOutput, puller, baseVertexShaderInvocation+i); // run VertexPuller
    ...
    vertexShader(primitive->vertices+i, &shaderInput, gpu); // run VertexShader
  }
