...
function insert_row($action, $test_name, $var1, $var2) {
  ...
  switch($action) {
    ...
    case 5:
      ...
      break;
    case 6:
      create_row('#DC143C', '#DC143C', $test_name, 'OK', 'FAILURE', '<b>'.$interpret.'</b> exit with code <b>'.$var1.'</b>, expected code is <b>'.$var2.'</b>');
      break;
  }
...
foreach ($src_files as $src_file) {
  ...
  if ($exit_code) {
    ...
    $exit_code = shell_exec('python3.6 '.escapeshellarg($interpret).' --source='.escapeshellarg($tmp_parse_name).' >'.escapeshellarg($tmp_int_name).' ; echo $?');
    $exit_code = intval($exit_code);
    if ($exit_code == $expected_code) {
      if (!$exit_code) {
...
foreach($dir_arr as $key => $stats_arr) {
  while($counter < count($stats_arr)) {
    $counter++;
    if ($stats_arr[$counter] || $stats_arr[$counter+1]) {
      $per_parse = $stats_arr[$counter] / ($stats_arr[$counter] + $stats_arr[$counter+1]) * 100;
      $per_sum = ($stats_arr[$counter+2] + $stats_arr[$counter+4]) / ($stats_arr[$counter] + $stats_arr[$counter+1]) * 100;
    }
    else {
      $per_parse = 0;
      $per_sum = 0;
    ...
    if ($stats_arr[$counter+2] && $stats_arr[$counter+3])  
      $per_int = $stats_arr[$counter+2] / ($stats_arr[$counter+2] + $stats_arr[$couter+3]) * 100;
    else
      $per_int = 0;
